...
def afterlogin_view(request):
    if is_student(request.user):      
        ...
        approved_acc = TMODEL.Teacher.objects.all().filter(user_id=request.user.id, status=True)  
        if approved_acc:   
...
# Admin student secion starts Here
...
# Admin student Section  Ends Here 
...
# Admin teacher section starts here
...
def admin_teacher_view(request):
    mydict={
        'total_teachers':TMODEL.Teacher.objects.all().filter(status=True).count(),
        'pending_teachers':TMODEL.Teacher.objects.all().filter(status=False).count(),
        'salary' : TMODEL.Teacher.objects.all().filter(status=True).aggregate(Sum('salary'))['salary__sum'],
    }
    return render(request,"exam/admin_teacher.html",context=mydict)
...
def update_teacher_view(request,pk):
    ...
    user = TMODEL.User.objects.get(id=teacher.user.user_id)
    ...
    if request.method == 'POST':
        ...
        if userForm.is_valid() and teacherForm.is_valid():
            ...
            user.save()
            teacherForm.save()
            return redirect('admin-view-teaher')
...
# Admin teacher section Ends Here 